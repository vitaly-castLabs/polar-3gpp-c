    ...
    for (num_bits = 15; num_bits <= 30; num_bits++) {
        ...
        for (i = 0; i < num_iterations; i++) {
            ...
            if (st != POLAR_STATUS_OK) {
                fprintf(stderr, "custom1 encode failed: status=%d\n", st);
                free(f_tilde);
                free(indices);
                return 1;
            }
            ...
            st = polar_custom1_decode(f_tilde, E, A, 8, 1, &dec);
            if (st != POLAR_STATUS_OK) {
                fprintf(stderr, "custom1 decode failed: status=%d\n", st);
                polar_free(enc.data);
                free(f_tilde);
                free(indices);
                return 1;
        ...
        printf("Scatter custom1: %d bits, decoding success: %.1f%%, wrong corrections: %.1f%%, repeat code success: %.1f%%\n",
               num_bits,
